...
// This code is a straight copy paste from https://github.com/mikebryant/ac-nh-turnip-prices with a sprinkling of type safety
...
function generate_individual_random_price(
  ...
  for (let i = start; i < start + length; i++) {
    ...
    if (!isNaN(given_prices[i])) {
      if (
        given_prices[i] < min_pred - FUDGE_FACTOR ||
        given_prices[i] > max_pred + FUDGE_FACTOR
      ) {
      ...
      const real_rate_range = rate_range_from_given_and_base(
...
class PDF {
  ...
  decay(rate_decay_min, rate_decay_max) {
    const ret = new PDF(
      this.min_value() - rate_decay_max,
      this.max_value() - rate_decay_min,
      false
    );
    /*
    ...
    */
    ...
    for (let x = 0; x < ret.prob.length; x++) {
      ...
      ret.prob[x] = sum(
        x + rate_decay_min + 1 + ret.value_start,
      ...
      ret.prob[x] += sum(
        x + rate_decay_min + ret.value_start,
...
function generate_peak_price(
  ...
  if (!isNaN(middle_price)) {
    ...
    if (
      middle_price < min_pred - FUDGE_FACTOR ||
      middle_price > max_pred + FUDGE_FACTOR
    ) {
      // Given price is out of predicted range, so this is the wrong pattern
      return 0;
    }
  ...
  for (const price of [left_price, right_price]) {
    ...
    // TODO: How to deal with probability when there's fudge factor?
    // Clamp the value to be in range now so the probability won't be totally biased to fudged values.
    const rate2_range = rate_range_from_given_and_base(
...
function* generate_all_patterns(sell_prices, previous_pattern) {
  ...
  for (let i = 0; i < 4; i++) {
    yield* multiply_generator_probability(
      generate_pattern_fns[i](sell_prices),
...
module.exports = function analyze_possibilities(
  sell_prices,
  first_buy,
  previous_pattern
) {
  const generated_possibilities = Array.from(
